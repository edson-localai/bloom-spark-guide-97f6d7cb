    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <Section title="Identificação">
            ...
            <Field label="Estágio do funil">
              <select className={inputCls} value={form.stage || 'novo'} onChange={(e) => set('stage', e.target.value as LeadStage)}>
          ...
          <Section title="Endereço">
            <Field label="CEP">
              <input className={inputCls} value={form.cep || ''} onChange={(e) => set('cep', e.target.value)} onBlur={handleCepBlur} placeholder="00000-000" />
            </Field>
            ...
            <Field label="UF">
              <input maxLength={2} className={inputCls} value={form.state || ''} onChange={(e) => set('state', e.target.value.toUpperCase())} />
            </Field>
          </Section>

          <Section title="Veículo">
            <Field label="Marca">
              <input className={inputCls} value={form.vehicle_brand || ''} onChange={(e) => set('vehicle_brand', e.target.value)} />
            </Field>
            <Field label="Modelo">
              <input className={inputCls} value={form.vehicle_model || ''} onChange={(e) => set('vehicle_model', e.target.value)} />
            </Field>
            <Field label="Ano">
              <input type="number" className={inputCls} value={form.vehicle_year || ''} onChange={(e) => set('vehicle_year', e.target.value ? Number(e.target.value) : null)} />
            </Field>
          </Section>
        ...
        <div className="p-4 border-t border-[#1F232E] flex justify-end gap-3 sticky bottom-0 bg-[#0F1117]">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm text-zinc-400 hover:text-white">Cancelar</button>
          <button onClick={save} disabled={saving} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500 text-black font-semibold text-sm hover:bg-cyan-400 disabled:opacity-50">
...
const inputCls = 'w-full bg-[#151821] border border-[#1F232E] rounded-lg px-3 py-2 text-sm text-zinc-200 focus:border-cyan-500/50 outline-none disabled:opacity-50';
