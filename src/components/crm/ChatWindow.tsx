...
function MediaPreview({ path, type, name }: { path: string; type: string; name: string }) {
  ...
  if (loading)
    return (
      <div className="h-20 flex items-center justify-center bg-black/10 rounded-lg">
        <Loader2 className="h-4 w-4 animate-spin text-cyan-500" />
      </div>
    );
  if (!signedUrl)
    return (
      <div className="p-2 text-[10px] text-red-400 italic bg-red-500/5 rounded">
    ...
    return (
      <div className="mb-2 rounded-lg overflow-hidden border border-white/10 cursor-pointer">
        <img
          src={signedUrl}
          alt="Mídia"
          className="max-w-full h-auto hover:scale-[1.02] transition-transform"
        />
      </div>
    );
  }
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="flex items-center gap-2 mb-2 p-2 rounded-lg bg-black/20 hover:bg-black/40 transition-colors"
    >
...
export function ChatWindow({ conversation }: ChatWindowProps) {
  ...
  const { messages, loading, sendMessage, deleteMessage, addEvent } = useMessages(
  ...
  const handleTransfer = async (agentId: string) => {
    ...
    try {
      ...
      await addEvent(
        `Conversa transferida para ${agents.find((a) => a.id === agentId)?.name || "outro agente"}`,
      );
  ...
  const handleScheduleMessage = async () => {
    ...
    try {
      ...
      toast.success(
        `Mensagem agendada para ${format(dateTime, "dd/MM 'às' HH:mm", { locale: ptBR })}`,
      );
      setInput("");
      setShowSchedule(false);
    } catch (error) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: "#0A0A0F" }}>
      {/* Header */}
      <div
        className="h-14 sm:h-16 px-3 sm:px-6 flex items-center justify-between border-b border-[#1F232E] gap-2"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">
              {conversation.contact?.name || conversation.whatsapp_chat_id}
            </p>
            <div className="flex items-center gap-1.5">
              {isAiLoading ? (
                <div className="flex gap-0.5 items-center">
                  ...
                  ></span>
                  <p className="text-[10px] text-cyan-400 ml-1 font-bold uppercase tracking-wider">
                    Ana pensando...
                  </p>
                </div>
              ) : (
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <div className="relative">
            ...
            >
              ...
              <ChevronDown
                className={`h-3 w-3 transition-transform ${showLabels ? "rotate-180" : ""}`}
              />
            </button>
            ...
            <AnimatePresence>
              {showLabels && (
                ...
                >
                  <p className="px-2 py-1 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                    Atribuir Etiquetas
                  </p>
          ...
          <button
            ...
            title={
              conversation.status === "resolved" ? "Reabrir atendimento" : "Resolver atendimento"
            }
          >
            {conversation.status === "resolved" ? (
                  ...
                  <button
                    ...
                    onClick={() =>
                      isOnline
                        ...
                        : toast.error(`${agent.name} está offline no momento.`)
                    }
                    className={`flex flex-col items-center gap-2 p-3 rounded-xl bg-black/20 border transition-all min-w-[100px] ${
                      isOnline
                        ? "border-[#1F232E] hover:border-cyan-500/30"
                        : "border-red-500/10 opacity-50 cursor-not-allowed"
                    }`}
                  >
                    <div
                      className={`h-10 w-10 rounded-full flex items-center justify-center font-bold relative ${
                        isOnline ? "bg-cyan-500/10 text-cyan-500" : "bg-zinc-800 text-zinc-600"
                      }`}
                    >
                      {agent.name.charAt(0)}
                      {isOnline && (
                        <span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full bg-emerald-500 border-2 border-[#151821]" />
                      )}
                    </div>
                    ...
                    <span
                      className={`text-[8px] uppercase font-bold tracking-tighter ${isOnline ? "text-emerald-500" : "text-zinc-600"}`}
                    >
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-xl bg-amber-500/5 border-2 border-dashed transition-all min-w-[100px] ${
                  onlineAgents.length === 0
                    ...
                    : "border-zinc-700/40 opacity-60 cursor-not-allowed"
                }`}
                disabled={onlineAgents.length > 0}
                title={
                  onlineAgents.length > 0
                    ? "Use somente quando não houver agentes online"
                    : "Adicionar à fila"
                }
              >
              ...
              {suggestions && (
                ...
                >
                  ...
                  <SuggestionButton
                    ...
                    onClick={() => {
                      setInput(suggestions.technical);
                      setSuggestions(null);
                    }}
                  />
                </motion.div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <span className="text-[9px] uppercase font-bold tracking-widest text-zinc-500 group-hover:text-cyan-400">
          {label}
        </span>
      </div>
      <p className="text-[11px] text-zinc-400 line-clamp-2 leading-relaxed group-hover:text-zinc-200">
        {text}
      </p>
    </button>
  );
