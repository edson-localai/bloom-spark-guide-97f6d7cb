...
export function ContactSidebar({
  ...
  return (
    <>
      <div
        className={`${variant === "mobile" ? "w-full" : "w-80 shrink-0"} h-full flex flex-col overflow-y-auto custom-scrollbar`}
        style={{
          background: "#0F1117",
          borderLeft: variant === "mobile" ? "none" : "1px solid #1F232E",
        }}
      >
        {/* Profile */}
        <div className="p-6 text-center border-b border-[#1F232E] relative">
          <button
            ...
            className="absolute top-3 right-3 p-1.5 text-zinc-500 hover:text-cyan-400 transition-colors"
          ...
          <span
            className={`inline-block mt-3 text-[9px] uppercase font-bold tracking-widest px-2 py-0.5 rounded border ${STAGE_COLORS[stage]}`}
          >
            {stage}
          </span>
        </div>
        ...
        <Section title="Dados pessoais">
          ...
          <Row
            icon={Calendar}
            label="Nascimento"
            value={c.birthdate ? format(new Date(c.birthdate), "dd/MM/yyyy") : null}
          />
        ...
        <Section title="Endereço">
          ...
          <Row
            icon={MapPin}
            label="Endereço"
            value={fullAddress.trim() ? fullAddress : null}
            multiline
          />
        ...
        <Section title="Veículo">
          <Row
            icon={Car}
            label="Marca/Modelo"
            value={[c.vehicle_brand, c.vehicle_model].filter(Boolean).join(" ") || null}
          />
        ...
        <div className="px-6 py-4 border-t border-[#1F232E] space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {(c.tags || []).map((tag) => (
              <span
                key={tag}
                className="group text-[10px] px-2 py-0.5 rounded bg-[#1F232E] text-zinc-300 border border-[#1F232E] flex items-center gap-1"
              >
                {tag}
                <button
            ...
            {addingTag ? (
              ...
              <button
                ...
                className="text-[10px] px-2 py-0.5 rounded border border-dashed border-zinc-700 text-zinc-500 hover:text-zinc-300 flex items-center gap-1"
              >
        ...
        <div className="p-6 space-y-2 border-t border-[#1F232E]">
          <a
            href={`/atendimento/propostas?contact=${c.id}`}
            className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl bg-[#151821] border border-[#1F232E] text-sm text-zinc-300 hover:border-cyan-500/30 hover:text-white transition-all group"
          >
      ...
      {editing && (
        <ContactEditor
          ...
          onSaved={(updates) =>
            setLocal((prev) => (prev ? ({ ...prev, ...updates } as Contact) : prev))
          }
        />
      )}
    </>
    ...
    return (
      <div className="flex items-center gap-2 text-zinc-700">
        ...
        <span className="text-[11px]">
          {label}: <span className="italic text-zinc-700">não informado</span>
        </span>
      </div>
    );
  return (
...
function Timeline({ contactId }: { contactId: string }) {
  ...
  if (loading)
    return <div className="text-[10px] text-zinc-600 animate-pulse">Carregando eventos...</div>;
  return (
...
function ScheduledList({ conversationId }: { conversationId?: string | null }) {
  const { scheduledMessages, loading, cancelMessage } = useScheduledMessages(
    conversationId || null,
  );
  if (loading && scheduledMessages.length === 0)
    return (
      <div className="text-[10px] text-zinc-600 animate-pulse">Carregando agendamentos...</div>
    );
  return (
          ...
          >
            <div className="flex justify-between items-start mb-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span
                  className={`text-[8px] font-bold uppercase tracking-widest ${msg.status === "pending" ? "text-amber-500" : msg.status === "sent" ? "text-emerald-500" : "text-zinc-600"}`}
                >
                  {msg.status}
                </span>
              </div>
