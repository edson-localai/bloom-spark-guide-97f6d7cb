...
function RespostasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return replies.filter(
      (r) =>
        !s || r.title.toLowerCase().includes(s) || (r.shortcut || "").toLowerCase().includes(s),
    );
  }, [replies, search]);
  ...
  const save = async () => {
    ...
    try {
      if (editing.id)
        await updateReply(editing.id, { title: editing.title, content: editing.content, shortcut });
      ...
      <div className="p-8 pb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            Use <span className="font-mono text-cyan-400">/atalho</span> no chat para inserir.
          </p>
        </div>
        <button
          onClick={() => setEditing({ title: "", content: "", shortcut: "" })}
          className="flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black px-4 py-2 rounded-lg font-semibold text-sm"
        >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 text-zinc-500 text-[10px] mt-0.5">
                        ...
                        <span className="bg-[#1F232E] px-1.5 py-0.5 rounded text-cyan-400 font-mono">
                          /{reply.shortcut}
                        </span>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-[#1F232E]">
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-2 text-zinc-500 hover:text-cyan-400"
                    >
            ...
            >
              <div className="p-5 border-b border-[#1F232E] flex items-center justify-between">
                <h3 className="text-lg font-bold text-white">
                  {editing.id ? "Editar" : "Nova"} resposta rápida
                </h3>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-[#151821] border border-[#1F232E] rounded-lg px-3 py-2 text-sm text-zinc-200"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                        shortcut: e.target.value.replace(/^\/+/, "").toLowerCase(),
                      })
                    }
                    placeholder="ex: ola"
                    className="w-full bg-[#151821] border border-[#1F232E] rounded-lg px-3 py-2 text-sm font-mono text-cyan-400"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">
              ...
              <div className="p-4 border-t border-[#1F232E] flex justify-end gap-3">
                ...
                <button
                  onClick={save}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500 text-black font-semibold text-sm hover:bg-cyan-400"
                >
