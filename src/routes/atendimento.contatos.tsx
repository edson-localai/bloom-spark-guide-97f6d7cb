...
function ContatosPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      toast.success(
        `Sincronização concluída! ${res.created} novos contatos, ${res.updated} atualizados.`,
      );
      fetchContacts();
    } catch (err: any) {
  ...
  const newContact = async () => {
    ...
    const { data, error } = await supabase
      .from("contacts")
      .insert({ phone: digits, stage: "novo" })
      .select("*")
      .single();
      ...
      <div className="p-8 pb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            Gerencie leads, dados pessoais, endereço e veículo.
          </p>
        </div>
                  ...
                  <tr key={contact.id} className="hover:bg-[#151821]/50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-zinc-300">
                        ...
                        <span>
                          {[
                            contact.vehicle_brand,
                            contact.vehicle_model,
                            contact.vehicle_year ? `(${contact.vehicle_year})` : "",
                          ]
                            .filter(Boolean)
                            .join(" ") || <span className="text-zinc-600 italic">—</span>}
                        </span>
                      </div>
                    </td>
