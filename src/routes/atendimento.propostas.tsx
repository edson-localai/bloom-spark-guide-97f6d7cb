...
function PropostasPage() {
    ...
    if (debouncedSearch) {
      ...
      result = proposals.filter((p) => {
        ...
        if (normalizedSearch.length > 0) {
          if (
            normalize(pNumber).includes(normalizedSearch) ||
  ...
  async function fetchAgents() {
    try {
      const { data, error } = await supabase.from("agents").select("user_id, name").order("name");
  ...
  const generatePDF = (proposal: any) => {
    ...
    doc.text(
      `Veículo: ${contact?.vehicle_brand || ""} ${contact?.vehicle_model || ""} ${contact?.vehicle_year ? `(${contact.vehicle_year})` : ""}`,
      20,
      68,
    );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "accepted":
        return (
          <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 uppercase">
            <CheckCircle2 className="h-2.5 w-2.5" /> Aprovada
          </span>
        );
      case "sent":
        return (
          <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded bg-blue-500/10 text-blue-500 border border-blue-500/20 uppercase">
            <SendHorizonal className="h-2.5 w-2.5" /> Enviada
          </span>
        );
      case "rejected":
        return (
          <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded bg-red-500/10 text-red-500 border border-red-500/20 uppercase">
            <X className="h-2.5 w-2.5" /> Rejeitada
          </span>
        );
      default:
        return (
          <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded bg-zinc-500/10 text-zinc-500 border border-zinc-500/20 uppercase">
                    ...
                    >
                      ...
                      {contacts.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.name || c.phone} {c.vehicle_brand ? `(${c.vehicle_brand})` : ""}
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    {items.map((item, index) => (
                      <div key={item.id} className="flex gap-4 items-start">
                        ...
                        <button
                          ...
                          className="mt-2.5 text-zinc-600 hover:text-red-500 transition-colors"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loadingProposals ? (
                ...
                filteredProposals.map((prop) => (
                  ...
                  >
                    <div className="absolute top-0 right-0 p-3">{getStatusBadge(prop.status)}</div>
                    ...
                    <h3 className="font-bold text-white mb-1 truncate pr-16">
                      {prop.contact?.name || "Cliente"}
                    </h3>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {prop.agent?.name && (
                        <div
                          className="flex items-center gap-1 text-[9px] text-zinc-600 font-bold uppercase bg-white/5 px-2 py-0.5 rounded"
                          title={`Vendedor: ${prop.agent.name}`}
                        >
